...
public class EngineAction {
	 ...
	 * 根据所有项目
	 * */
	@GetMapping("selectAll7/{p}/{name}")
	public PageInfo<Shop> SelectAll7(@PathVariable Integer p,@PathVariable String name){
		if(name.equals("-1")) {
			name="";
		}
		return biz.SelectAll7(p,name);
	}
	 ...
	 * */
	@PostMapping("addShop")
	public int addShop(@RequestBody Shop s) {
		return biz.addShop(s);
	}
